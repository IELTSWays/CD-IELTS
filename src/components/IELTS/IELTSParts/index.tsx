const partsData = {
  reading: [
    ...
    { title: "Part 2", description: "Read the text and answer questions 14-26." },
    { title: "Part 3", description: "Read the text and answer questions 27-40." },
  ],
  listening: [
...
const Parts = ({ part }) => {
  const partInfo = partsData[localStorage.getItem('test_skill')]?.[part - 1];
    ...
    <div className='ielts-container pt-10'>
      {partInfo && (
        <div className='ielts-title'>
          ...
          {localStorage.getItem('test_name')}
        </div>
      )}
    </div>
  );
