...
const Q29 = ({ qn }: any) => {
  ...
  const [item, setItem] = useState(answersAll['00029']);
  ...
  const [{ hover }, drop] = useDrop(() => ({
    ...
    drop: (item: any, _monitor) => {
      setItem(item);
      dispatch(setAnswersAll(Object.assign({}, answersAll, { '00029': item })))
  ...
  return (
    ...
    >
      <Paper elevation={0} className="drop-container-text">
        <Typography>Little is done in the hospitality industry to help workers improve their skills.</Typography>
      </Paper>
      <Paper elevation={0}>
        <Box ref={drop} >
          <Paper
