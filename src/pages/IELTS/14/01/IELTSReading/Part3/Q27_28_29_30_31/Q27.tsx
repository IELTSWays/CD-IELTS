...
const Q27 = ({ qn }: any) => {
  ...
  const [item, setItem] = useState(JSON.parse((localStorage.getItem('00027'))));
  const dispatch = useAppDispatch();

  console.log(JSON.parse((localStorage.getItem('00027'))))
  ...
  const postAnswer = useQuery({
    ...
    queryFn: async () => {
      const response = await axiosInstance.post(`exam/answer/${localStorage.getItem('test_id')}`, {
        "test_done": false,
        "answers": {
          "00027": localStorage.getItem('00027')
        }
      })
  ...
  const [{ hover }, drop] = useDrop(() => ({
    ...
    drop: (data: any, _monitor) => {
      setItem(data);
      localStorage.setItem('00027', JSON.stringify(data));
      postAnswer.refetch()
