...
const Q31 = ({ qn }: any) => {
  ...
  console.log(JSON.parse((localStorage.getItem('00031'))))
  ...
  const postAnswer = useQuery({
    ...
    queryFn: async () => {
      const response = await axiosInstance.post(`exam/answer/${localStorage.getItem('test_id')}`, {
        "test_done": false,
        "answers": {
          "00031": localStorage.getItem('00031')
        }
      })
  ...
  const [{ hover }, drop] = useDrop(() => ({
    ...
    drop: (data: any, _monitor) => {
      setItem(data);
      localStorage.setItem('00031', JSON.stringify(data));
      postAnswer.refetch()
  ...
  useEffect(() => {
    localStorage.setItem('00031', JSON.stringify(item));
  }, [item]);
