...
const DND_17_20 = () => {
  ...
  const init = {
    tasks: {
      "a": { id: "a", content: "A - It has a good place to stop and rest." },
      "b": { id: "b", content: "B - It is suitable for all abilities." },
      "c": { id: "c", content: "C - It involves crossing a river." },
      "d": { id: "d", content: "D - It demands a lot of skill." },
      "e": { id: "e", content: "E - It may be closed in bad weather." },
      "f": { id: "f", content: "F - It has some very narrow sections." },
    },
    columns: {
      "column-1": {
        id: "column-1",
    ...
    columnOrder: ["column-1", 17, 18, 19, 20]
  }
  ...
  useEffect(() => {
    localStorage.setItem('17', Object.values(starter.columns)[0]?.taskIds);
    localStorage.setItem('18', Object.values(starter.columns)[1]?.taskIds);
    localStorage.setItem('19', Object.values(starter.columns)[2]?.taskIds);
    localStorage.setItem('20', Object.values(starter.columns)[3]?.taskIds);
  }, [starter]);

  useEffect(() => {
    setAnswer17(Object.values(starter.columns)[0]?.taskIds)
    setAnswer18(Object.values(starter.columns)[1]?.taskIds)
    setAnswer19(Object.values(starter.columns)[2]?.taskIds)
    setAnswer20(Object.values(starter.columns)[3]?.taskIds)
  }, [starter])

  useEffect(() => {
    answer17 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '17': (Object.values(starter.columns)[0]?.taskIds).join("") })))
  }, [answer17])

  useEffect(() => {
    answer18 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '18': (Object.values(starter.columns)[1]?.taskIds).join("") })))
  }, [answer18])

  useEffect(() => {
    answer19 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '19': (Object.values(starter.columns)[2]?.taskIds).join("") })))
  }, [answer19])

  useEffect(() => {
    answer20 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '20': (Object.values(starter.columns)[3]?.taskIds).join("") })))
  }, [answer20])
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <Droppable droppableId="all-column" type="column" direction="horizontal">
        {(provided, snapshot) => (
          ...
          >
            <section className="dnd-cols" id={`q-17`}>
