...
const DND_15_20 = () => {
  ...
  const init = {
    tasks: {
      "a": { id: "a", content: "A - not a permanent job" },
      "b": { id: "b", content: "B - involves leading a team" },
      "c": { id: "c", content: "C - experience not essential" },
      "d": { id: "d", content: "D - intensive work but also fun" },
      "e": { id: "e", content: "E - chance to earn more through overtime" },
      "f": { id: "f", content: "F - chance for rapid promotion" },
      "g": { id: "g", content: "G - accommodation available" },
      "h": { id: "h", content: "H - local travel involved" }
    },
    columns: {
      "column-1": {
        id: "column-1",
      ...
      15: {
        id: 15,
        title: "Fresh food commercial manager",
    ...
    columnOrder: ["column-1", 15, 16, 17, 18, 19, 20]
  }
  ...
  useEffect(() => {
    localStorage.setItem('15', Object.values(starter.columns)[0]?.taskIds);
    localStorage.setItem('16', Object.values(starter.columns)[1]?.taskIds);
    localStorage.setItem('17', Object.values(starter.columns)[2]?.taskIds);
    localStorage.setItem('18', Object.values(starter.columns)[3]?.taskIds);
    localStorage.setItem('19', Object.values(starter.columns)[4]?.taskIds);
    localStorage.setItem('20', Object.values(starter.columns)[5]?.taskIds);
  }, [starter]);

  useEffect(() => {
    setAnswer15(Object.values(starter.columns)[0]?.taskIds)
    setAnswer16(Object.values(starter.columns)[1]?.taskIds)
    setAnswer17(Object.values(starter.columns)[2]?.taskIds)
    setAnswer18(Object.values(starter.columns)[3]?.taskIds)
    setAnswer19(Object.values(starter.columns)[4]?.taskIds)
    setAnswer20(Object.values(starter.columns)[5]?.taskIds)
  }, [starter])

  useEffect(() => {
    answer15 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '15': (Object.values(starter.columns)[0]?.taskIds).join("") })))
  }, [answer15])

  useEffect(() => {
    answer16 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '16': (Object.values(starter.columns)[1]?.taskIds).join("") })))
  }, [answer16])

  useEffect(() => {
    answer17 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '17': (Object.values(starter.columns)[2]?.taskIds).join("") })))
  }, [answer17])

  useEffect(() => {
    answer18 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '18': (Object.values(starter.columns)[3]?.taskIds).join("") })))
  }, [answer18])

  useEffect(() => {
    answer19 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '19': (Object.values(starter.columns)[4]?.taskIds).join("") })))
  }, [answer19])

  useEffect(() => {
    answer20 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '20': (Object.values(starter.columns)[5]?.taskIds).join("") })))
  }, [answer20])
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <Droppable droppableId="all-column" type="column" direction="horizontal">
        {(provided, snapshot) => (
          ...
          >
            <section className="dnd-cols" id={`q-15`}>
