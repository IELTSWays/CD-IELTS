...
const DND_27_27 = () => {
  ...
  const init = {
    tasks: {
      "a": { id: "a", content: "A - Tim found this easier than expected." },
      "b": { id: "b", content: "B - Tim thought this was not very clearly organised." },
      "c": { id: "c", content: "C - Diana may do some further study on this." },
      "d": { id: "d", content: "D - They both found the reading required for this was difficult." },
      "e": { id: "e", content: "E - Tim was shocked at something he learned on this module." },
      "f": { id: "f", content: "F - They were both surprised how little is known about some aspects of this." },
    },
    columns: {
      "column-1": {
        id: "column-1",
        title: "",
        taskIds: ["a", "b", "c", "d", "e", "f"].filter(x =>
  ...
  useEffect(() => {
    localStorage.setItem('27', Object.values(starter.columns)[0]?.taskIds);
    localStorage.setItem('28', Object.values(starter.columns)[1]?.taskIds);
    localStorage.setItem('29', Object.values(starter.columns)[2]?.taskIds);
    localStorage.setItem('30', Object.values(starter.columns)[3]?.taskIds);
  }, [starter]);

  useEffect(() => {
    setAnswer27(Object.values(starter.columns)[0]?.taskIds)
    setAnswer28(Object.values(starter.columns)[1]?.taskIds)
    setAnswer29(Object.values(starter.columns)[2]?.taskIds)
    setAnswer30(Object.values(starter.columns)[3]?.taskIds)
  }, [starter])

  useEffect(() => {
    answer27 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '27': (Object.values(starter.columns)[0]?.taskIds).join("") })))
  }, [answer27])

  useEffect(() => {
    answer28 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '28': (Object.values(starter.columns)[1]?.taskIds).join("") })))
  }, [answer28])

  useEffect(() => {
    answer29 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '29': (Object.values(starter.columns)[2]?.taskIds).join("") })))
  }, [answer29])

  useEffect(() => {
    answer30 && dispatch(setAnswersAll(Object.assign({}, answersAll, { '30': (Object.values(starter.columns)[3]?.taskIds).join("") })))
  }, [answer30])
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <Droppable droppableId="all-column" type="column" direction="horizontal">
        {(provided, snapshot) => (
          ...
          >
            <section className="dnd-cols" id={`q-27`}>
